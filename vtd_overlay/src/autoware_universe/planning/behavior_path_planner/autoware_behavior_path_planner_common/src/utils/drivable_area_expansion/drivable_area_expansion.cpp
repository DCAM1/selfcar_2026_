...
namespace
{
constexpr double kTurnCurvatureThreshold = 1.0e-4;
  ...
  const DrivableAreaExpansionParameters & params)
{
  ...
  expansion.curvatures = curvatures;
  for (auto path_idx = 0UL; path_idx < path_poses.size(); ++path_idx) {
    if (curvatures[path_idx] == 0.0) continue;
    const auto curvature_radius = 1 / std::abs(curvatures[path_idx]);
    auto effective_params = params;
    if (curvatures[path_idx] >= -kTurnCurvatureThreshold) {
      // The VTD-specific extra clearance is only needed on right turns. Keep Autoware's original
      // swept-width calculation on straight and left-turn sections.
      effective_params.extra_width = 0.0;
    }
    expansion.min_lane_widths[path_idx] =
      calculate_minimum_lane_width(curvature_radius, effective_params);
  }
  ...
  const std::vector<Pose> & poses, const size_t smoothing_window_size)
{
  ...
  for (auto i = 0UL; i < curvatures.size(); ++i) {
    ...
    for (auto j = from_idx; j <= to_idx; ++j) sum += curvatures[j];
  ...
  const std::vector<double> & max_right_expansions)
{
  ...
  for (auto path_idx = 0UL; path_idx < expansion.min_lane_widths.size(); ++path_idx) {
    ...
    if (expansion.curvatures[path_idx] < -kTurnCurvatureThreshold) {
      // On a right turn the swept front-left corner is the outermost point. Apply the requested
      // clearance to the left bound first and use the inner bound only if the outer limit is hit.
      const auto left_expansion = std::min(left_limit, expansion_dist);
      const auto right_expansion = std::min(right_limit, expansion_dist - left_expansion);
      expansion.left_distances[left_bound_idx] = left_expansion;
      expansion.left_distances[left_bound_idx + 1] = left_expansion;
      expansion.right_distances[right_bound_idx] = right_expansion;
      expansion.right_distances[right_bound_idx + 1] = right_expansion;
      continue;
    }
