  ...
  const std::shared_ptr<RouteHandler> & route_handler)
{
  ...
  const bool is_in_road_lane = std::invoke([&]() {
    ...
    // A route lane can contain both its opening taper and a long parallel section. The reference
    // path delays the lateral shift until the taper is complete, so the ego temporarily follows a
    // straight sibling branch of the route lane. Accept only that geometrically matched sibling;
    // otherwise the route check interrupts planning while the ego is following the valid corridor.
    ...
    const auto get_exit_yaw = [](const lanelet::ConstLanelet & lane) -> std::optional<double> {
      const auto & centerline = lane.centerline();
      if (centerline.size() < 2) {
        return std::nullopt;
      }
    ...
    const auto get_chord_yaw = [](const lanelet::ConstLanelet & lane) -> std::optional<double> {
      const auto & centerline = lane.centerline();
      if (centerline.size() < 2) {
        return std::nullopt;
      }
    ...
    const auto yaw_difference = [](const double a, const double b) {
      return std::abs(std::atan2(std::sin(a - b), std::cos(a - b)));
    };
    const auto same_maneuver = [](const auto & route_lane, const auto & candidate) {
      ...
      return (!is_turning(route_turn) && !is_turning(candidate_turn)) ||
             route_turn == candidate_turn;
    };
    const auto are_parallel = [&](const auto & route_lane, const auto & candidate) {
      const auto route_entry = get_entry_yaw(route_lane);
      const auto route_exit = get_exit_yaw(route_lane);
      const auto candidate_entry = get_entry_yaw(candidate);
      const auto candidate_exit = get_exit_yaw(candidate);
      constexpr double max_yaw_difference = 5.0 * M_PI / 180.0;
      return same_maneuver(route_lane, candidate) && route_entry && route_exit && candidate_entry &&
             candidate_exit &&
             yaw_difference(*route_entry, *candidate_entry) <= max_yaw_difference &&
             yaw_difference(*route_exit, *candidate_exit) <= max_yaw_difference;
    };
    const auto has_common_split_predecessor = [&](const auto & route_lane, const auto & candidate) {
      for (const auto & route_previous : route_handler->getPreviousLanelets(route_lane)) {
        const auto route_roots = route_handler->getPreviousLanelets(route_previous);
        for (const auto & candidate_previous : route_handler->getPreviousLanelets(candidate)) {
          const auto candidate_roots = route_handler->getPreviousLanelets(candidate_previous);
          const bool has_common_root =
            std::any_of(route_roots.begin(), route_roots.end(), [&](const auto & route_root) {
              return std::any_of(
                candidate_roots.begin(), candidate_roots.end(), [&](const auto & candidate_root) {
                  return route_root.id() == candidate_root.id();
                });
            });
          if (has_common_root) {
            return true;
    ...
    // The delayed path can remain on the parallel successor of a short split connector for a few
    // meters. Accept it only when both lanes originate from the same split and are geometrically
    // parallel, rather than accepting arbitrary adjacent lanes.
    for (const auto & candidate :
         {route_handler->getLeftLanelet(closest_road_lane, true, false),
          route_handler->getRightLanelet(closest_road_lane, true, false)}) {
      if (
        candidate && autoware::experimental::lanelet2_utils::is_in_lanelet(self_pose, *candidate) &&
        are_parallel(closest_road_lane, *candidate) &&
        has_common_split_predecessor(closest_road_lane, *candidate)) {
        return true;
      }
    }

    const auto route_entry_yaw = get_entry_yaw(closest_road_lane);
    const auto route_exit_yaw = get_exit_yaw(closest_road_lane);
    const auto route_chord_yaw = get_chord_yaw(closest_road_lane);
    if (
      !route_entry_yaw || !route_exit_yaw || !route_chord_yaw ||
      closest_road_lane.centerline3d().empty()) {
      return false;
    }
    constexpr double minimum_straightness_improvement = 5.0 * M_PI / 180.0;
    for (const auto & previous : prev_lanes) {
      const auto incoming_yaw = get_exit_yaw(previous);
      if (!incoming_yaw) {
        continue;
      }
      const double route_straightness = yaw_difference(*route_entry_yaw, *incoming_yaw) +
                                        yaw_difference(*route_chord_yaw, *incoming_yaw);
      for (const auto & sibling : route_handler->getNextLanelets(previous)) {
        if (
          sibling.id() == closest_road_lane.id() ||
          sibling.attributeOr(lanelet::AttributeName::Subtype, std::string{}) != "road" ||
          !same_maneuver(closest_road_lane, sibling) ||
          !autoware::experimental::lanelet2_utils::is_in_lanelet(self_pose, sibling)) {
          continue;
        }
        const auto sibling_entry_yaw = get_entry_yaw(sibling);
        const auto sibling_exit_yaw = get_exit_yaw(sibling);
        const auto sibling_chord_yaw = get_chord_yaw(sibling);
        const bool has_parallel_exit =
          sibling_exit_yaw &&
          yaw_difference(*sibling_exit_yaw, *route_exit_yaw) <= 5.0 * M_PI / 180.0;
        const auto route_next_lanes = route_handler->getNextLanelets(closest_road_lane);
        const bool has_parallel_successors = std::any_of(
          route_next_lanes.begin(), route_next_lanes.end(), [&](const auto & route_next) {
            if (!route_handler->isRouteLanelet(route_next)) {
              return false;
            }
            const auto sibling_next = route_handler->getNextLanelets(sibling);
            return std::any_of(sibling_next.begin(), sibling_next.end(), [&](const auto & next) {
              return are_parallel(route_next, next);
            });
          });
        if (
          !sibling_entry_yaw || !sibling_exit_yaw || !sibling_chord_yaw ||
          (!has_parallel_exit && !has_parallel_successors)) {
          continue;
        }
        const double sibling_straightness = yaw_difference(*sibling_entry_yaw, *incoming_yaw) +
                                            yaw_difference(*sibling_chord_yaw, *incoming_yaw);
        if (sibling_straightness + minimum_straightness_improvement >= route_straightness) {
          continue;
        }
        const auto route_centerline = closest_road_lane.centerline3d();
        const auto sibling_centerline = sibling.centerline3d();
        const auto & route_end = route_centerline.back();
        const auto & sibling_end = sibling_centerline.back();
        const double dx = sibling_end.x() - route_end.x();
        const double dy = sibling_end.y() - route_end.y();
        const double longitudinal_offset =
          std::cos(*incoming_yaw) * dx + std::sin(*incoming_yaw) * dy;
